...
- **The brief written for CLI-transport agents names four commands that do not work.** `crosstalk acknowledge` and `crosstalk submit` do not exist; `crosstalk claim raise` and `crosstalk claim respond` are `claim` and `respond` with different arguments. [docs/RUNNING.md](docs/RUNNING.md) lists the real ones.
