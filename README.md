...
Agents from different vendors, including Claude Code, Codex and Cursor, build the same project together, argue about it on the record, and settle disagreements with evidence instead of rank.
...
**The leader isn't an oracle.** In the session that motivated this project, roughly as many defects came from the orchestrating agent, including misread code and briefs that contradicted each other, as from either worker. When findings are treated as orders, leader error becomes worker rework, and sometimes turns correct code into broken code.
...
- **Evidence expires.** Every result carries the commit it was gathered at. When the base moves, it goes stale and the claim reopens, so there is no more "passing" on code that no longer exists.
...
- **`taskAcceptance.method` only works as `leader` or `human`.** `majority` and `unanimous` are now refused with `NOT_TASK_AUTHORITY` rather than stranding silently, but the remedy named by that refusal is to open a decision and let its outcome carry, and that remedy does not exist yet: nothing maps a resolved decision onto a task state. `doctor` still does not refuse the config. Use `leader`.
...
- **A refused hub looks like a working, idle one.** It renders the full interface, including the channel list, composer and live Send button, over an empty log. The banner says `offline` and the Participants panel is empty; the event count does not distinguish the two, because a healthy new hub also reads `0 events`.
...
- A git repository **with at least one commit**. `init` does not check this and
  will set up a repository that `doctor` and `up` then both refuse ([#23](https://github.com/moazessam376-dev/crosstalk/issues/23)).
- At least one agent you can paste a line into. **Two workers** let you use the full
...
No compiler, no Python, no Docker, and no native modules are needed on Windows, macOS and Linux alike. Two runtime dependencies, total. `crosstalk doctor` checks all of it and names the remedy for anything missing.
...
every interface, which is useful for reading the hub from a phone, and says out loud
