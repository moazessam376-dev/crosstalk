...
- **`TASK_TRANSITIONS` is NOT widened.** Revision 2 said `submitted` gains `'in_progress'`; that was wrong and had already been ruled wrong. `validateTransition` governs what the daemon accepts *from clients*, so widening it makes `submitted → in_progress` legal for a participant-authored `task_state` — letting the assignee pull its own task out of the review queue at will, with nothing recording that a rebase happened. **The projection folds without validating**, so `rebase_notice` moves the task while the table keeps clients out. The widening buys nothing and costs the guarantee.
  Consequence, carried over: the daemon emits `rebase_notice` and **must not** also emit `task_state: in_progress`. That event is illegal by its own transition list and the fixture guard is right to reject it.
- `evidence_stale` marks the item; if the claim is `resolved` and **any** evidence item is stale, return it to `open` and clear `resolution`. Not "all stale": `Claim.evidence` is one flat array mixing the raiser's support, counter-evidence and the accepter's fix evidence, and `Evidence` carries no field distinguishing them. Under "all stale", a fix at `sha-B` that is rebased away leaves the claim resolved because the raiser's `sha-A` is still fresh — a resolution nobody can verify, which is the exact class §5.4 exists to eliminate. §5.4's word is *"solely"*, and **"any stale" is the conservative approximation of "solely" given an array that cannot tell fix evidence from support**. The asymmetry decides it: a false reopen costs one cheap re-run, a false stay-resolved hides a regression behind a green record.
- Reopen to `open`, never `contested` — `upheld` comes only from `accept`, so nobody contested, and `validateResponseAuthority` requires `claim.raisedBy` on `contested`, which would demand a response from the wrong participant. `rounds` is not reset.

> **Read [`docs/plans/2026-08-10-stale-evidence-recovery.md`](2026-08-10-stale-evidence-recovery.md) before touching A5.** It is FROZEN, it covers this same spec section, it was reviewed with fourteen findings all upheld, and revision 2 of this plan contradicted it in two places without ever citing it. Where the two disagree, **that plan wins** and this one is the defect.
...
**Acceptance.** A resolved claim reopens with `resolution` cleared when **any** evidence item goes stale — including the case that decides it: raiser's `sha-A` fresh, accepter's fix at `sha-B` orphaned. A claim with **no** stale evidence stays resolved; that is the other side of the discrimination. `validateTransition('submitted' → 'in_progress')` still **fails** — the projection moves the task, the table does not. `submitted` + `rebase_notice` → `in_progress`. `concede` on an escalated claim closes the decision and disarms its timer. Tests build a real throwaway repo under `os.tmpdir()` and rebase it — **do not mock git**.
